...
class TestSchedule(APITestCase):
    def setUp(self):
        ...
        self.class_3 = dbh.create_class(
            ...
            classroom = 'PJC',
            schedule = '24T45',
            days = ['Segunda-Feira 16:00 às 17:50', 'Quarta-Feira 16:00 às 17:50'],
            _class = "3",
            special_dates=[],
            discipline = self.discipline_1
        )
    ...
    def test_with_correct_parameters(self):
        ...
        schedule_generator = ScheduleGenerator(classes_id=[self.class_1.id, self.class_2.id, self.class_3.id, self.class_4.id], preference=[3, 2, 1])
    ...
    def test_with_higher_classes_limit(self):
        ...
        try:
            schedule_generator = ScheduleGenerator(classes_id=[self.class_1.id, self.class_2.id, self.class_3.id, self.class_4.id, self.class_5.id], preference=[3, 2, 1])
    ...
    def test_with_conflicting_classes(self):
        ...
        schedule_generator = ScheduleGenerator(classes_id=[self.class_4.id, self.class_6.id])
        schedules = schedule_generator.generate()
    ...
    def test_with_invalid_class(self):
        ...
        try:
            schedule_generator = ScheduleGenerator(classes_id=classes_ids + [random_id])
        except Exception as error:
            self.assertEqual(str(error), f"class with id {random_id} does not exist.")
